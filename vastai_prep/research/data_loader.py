...
from .augmentations import apply_skeleton_augmentations
...
def augment_training_set(X, y, multiplier=9):
    """
    Generate custom skeleton-aware augmented samples from the training set.
    Multiplies the dataset size by `multiplier`.
    """
    logger.info(f"Augmenting training set: current size={X.shape[0]} samples, multiplier={multiplier}")
    X_aug = [X]
    y_aug = [y]
    for m in range(multiplier - 1):
        X_batch = []
        for i in range(X.shape[0]):
            # squeeze channel to make 2D
            img_2d = X[i].squeeze()
            aug_img_2d = apply_skeleton_augmentations(img_2d)
            X_batch.append(np.expand_dims(aug_img_2d, axis=-1))
        X_aug.append(np.array(X_batch, dtype=np.float32))
        y_aug.append(y)
    
    X_res = np.concatenate(X_aug, axis=0)
    y_res = np.concatenate(y_aug, axis=0)
    logger.info(f"Augmented training set created: final size={X_res.shape[0]} samples")
    return X_res, y_res
