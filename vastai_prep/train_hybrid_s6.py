...
def build_teacher_model(num_classes, input_shape=(32, 32, 1)):
    """A heavy CNN model representing the Teacher (~9.3M parameters)"""
    inputs = layers.Input(shape=input_shape)
    ...
    x = layers.BatchNormalization()(x)
    x = layers.Dropout(0.4)(x)
    outputs = layers.Dense(num_classes)(x) # Note: linear activation because distiller uses custom softmax scale
