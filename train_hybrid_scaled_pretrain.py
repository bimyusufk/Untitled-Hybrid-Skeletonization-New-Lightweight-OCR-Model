...
WEIGHTS_PATH = "research_outputs/models/pretrained_se_64x64_width24.weights.h5"
...
def build_student_model_64x64_width24(num_classes, input_shape=(64, 64, 1)):
    inputs = layers.Input(shape=input_shape)
    
    # Block 1 (24 filters)
    x = layers.Conv2D(24, (3, 3), activation='relu', padding='same', dilation_rate=2)(inputs)
    ...
    # Block 2 (48 filters)
    x = layers.Conv2D(48, (3, 3), activation='relu', padding='same', dilation_rate=2)(x)
    x = layers.BatchNormalization()(x)
    x = layers.Conv2D(48, (3, 3), strides=(2, 2), activation='relu', padding='same')(x)
    x = layers.BatchNormalization()(x)
    x = se_block(x, ratio=8)
    x = layers.Dropout(0.2)(x)
    
    # Block 3 (96 filters)
    x = layers.Conv2D(96, (3, 3), activation='relu', padding='same', dilation_rate=2)(x)
    x = layers.BatchNormalization()(x)
    ...
    # Block 4 (192 filters)
    x = layers.Conv2D(192, (3, 3), activation='relu', padding='same', dilation_rate=2)(x)
    x = layers.BatchNormalization()(x)
    x = layers.Conv2D(192, (3, 3), strides=(2, 2), activation='relu', padding='same')(x)
    x = layers.BatchNormalization()(x)
    x = se_block(x, ratio=8)
    x = layers.Dropout(0.3)(x)
...
def main():
    print("=== STARTING S8 SYNTHETIC PRE-TRAINING FOR WIDTH-24 MODEL (64x64) ===")
    ...
    # Build student
    student = build_student_model_64x64_width24(NUM_CLASSES)
    ...
    print(f"Pre-training student width-24 model on {X_train.shape[0]} synthetic samples...")
    t0 = time.time()
    student.fit(
        X_train, y_train,
        epochs=EPOCHS,
    ...
    print(f"Saved pre-trained width-24 weights to {WEIGHTS_PATH}")
