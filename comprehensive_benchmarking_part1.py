# comprehensive_benchmarking_part1.py
import os
import time
import random
import json
...
# Logging helper
LOG_FILE = "comprehensive_benchmarking_part1.log"
def log_message(msg):
...
def main():
    t_start = time.time()
    log_message("=== STARTING COMPREHENSIVE OCR BENCHMARK (PART 1) ===")
    ...
    configs = [
        ("LeNet-5", lambda num_classes: LeNet5(num_classes), False),
    ...
    part_csv = os.path.join(output_dir, "comprehensive_benchmark_results_part1.csv")
    df.to_csv(part_csv, index=False)
    log_message(f"Part 1 results saved to {part_csv}")
    
    # Try to merge
    merge_results(output_dir)
    
    elapsed_time = time.time() - t_start
    log_message(f"=== COMPREHENSIVE OCR BENCHMARK PART 1 COMPLETED IN {elapsed_time/60:.2f} MINUTES ===")
